...
export async function validateAttestationBinding(
  ...
  if (
    !sameHex(result.sessionId, session.sessionId) ||
    !sameHex(attestation.sessionId, session.sessionId)
  ) {
    return "The signed session does not match the active verification capability.";
  }
  if (
    !sameHex(result.assetId, session.assetId) ||
    !sameHex(attestation.assetId, session.assetId)
  ) {
    return "The signed asset does not match the requested physical baseline.";
  }
  if (
    expectedFingerprintHash &&
    !sameHex(attestation.fingerprintHash, expectedFingerprintHash)
  ) {
  ...
  if (
    expectedRegistry &&
    !sameHex(signed.domain.verifyingContract, expectedRegistry)
  ) {
    return "The attestation domain does not match the configured registry.";
  }
  if (
    !sameHex(hashAliveAttestation(attestation, signed.domain), signed.digest)
  ) {
    return "The attestation digest does not match its EIP-712 payload.";
  }
  try {
    const recovered = await recoverAliveAttestationSigner(
      attestation,
      signed.domain,
      signed.signature,
    );
    if (!sameHex(recovered, signed.signer))
      return "The EIP-712 signature does not match the reported signer.";
