...
/**
 * Trigger-oriented autonomous Agent scheduled cycle (Internal Cron Handler).
 *
 ...
 */
async function runAgentEvaluation(ctx: any) {
...
export const scheduledAgentCycle = internalMutation({
  args: {},
  handler: async (ctx) => runAgentEvaluation(ctx),
});

export const triggerAgentCycle = mutation({
  args: {},
  handler: async (ctx) => runAgentEvaluation(ctx),
});
