...
function frame(width: number, height: number, value: (x: number, y: number) => number): Uint8ClampedArray {
...
describe("capture quality", () => {
  it("rejects a flat frame as unfocused", () => {
    const result = assessFrameQuality(frame(12, 12, () => 128), 12, 12);
    expect(result.blurScore).toBe(0);
    expect(result.usable).toBe(false);
  });

  it("detects detail in a balanced checkerboard", () => {
    const result = assessFrameQuality(frame(12, 12, (x, y) => ((x + y) % 2 ? 72 : 184)), 12, 12);
  ...
  it("rejects mismatched image dimensions", () => {
    expect(() => assessFrameQuality(new Uint8ClampedArray(4), 8, 8)).toThrow(/dimensions/);
  });
});
