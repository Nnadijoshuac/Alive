...
type Stage =
  "loading" | "preparing" | "ready" | "running" | "restoring" | "error";
...
export function AttackLabPage() {
  ...
  const [onchainUnavailableReason, setOnchainUnavailableReason] =
    useState<string>();
  ...
  const prepareBaseline = useCallback(async () => {
    ...
    setError(undefined);
    setProgress([
      "Resetting demo overrides...",
      "Loading issuer documentation...",
      "Extracting facts...",
    ...
    try {
      ...
      setProgress((current) => [
        ...current,
        "Publishing verdict to X Layer Testnet...",
        "Attempting gated deposit...",
      ]);
      await runOnchain();
      ...
      if (v.status !== "ELIGIBLE") {
        setError(
          new Error(
            `Baseline is not ELIGIBLE (${v.status}): ${v.reasons.map((r) => r.code).join(", ")}`,
          ),
        );
      }
    } catch (requestError) {
  ...
  function xLayerLabel(): string {
    if (onchainState === "unavailable")
      return isEligible
        ? "Ready (unproven)"
        : "Restricted; onchain proof not run";
    if (onchainState === "not-run")
      return isEligible ? "Ready" : "Restricted; onchain proof not run";
    ...
    <div className={overviewStyles.page}>
      <header className={overviewStyles.pageHeader}>
        <div>
          ...
          <h1 className={overviewStyles.heading}>
            Test the rule boundary without touching a live asset.
          </h1>
          <p className={overviewStyles.subheading}>
            Attack Lab changes only the labelled TTBILL-A fixture, re-runs
            deterministic eligibility, and reports whether an X Layer testnet
            proof actually ran.
          </p>
        </div>
        <div className={overviewStyles.methodLedger}>
          ...
          <ol>
            <li>
              <b>Fixture</b>
              <small>TTBILL-A only</small>
            </li>
            <li>
              <b>31h</b>
              <small>Simulated NAV age</small>
            </li>
            <li>
              <b>{onchainState === "done" ? "Proven" : "Unproven"}</b>
              <small>Onchain state</small>
            </li>
          </ol>
        </div>
      </header>
      ...
      <div className={styles.assetCard}>
        <div className={styles.assetIdentity}>
          ...
          <span className={styles.assetName}>
            X Layer Testnet Enforcement Harness
          </span>
          <span className={styles.demoBadge}>
            Testnet harness; not a real asset
          </span>
        </div>
        {quote ? (
            ...
            <div className={styles.stateCell}>
              ...
              <span
                className={`${styles.stateValue} ${
                  isEligible
                    ? styles.stateValuePositive
                    : isRestricted
                      ? styles.stateValueNegative
                      : ""
                }`}
              >
      ...
      <div className={styles.scenarioCard}>
        ...
        <p className={styles.scenarioDescription}>
          Push the demo asset&apos;s NAV beyond ALIVE&apos;s permitted freshness
          window (24 hours). Expect ALIVE to detect NAV_STALE, publish a
          RESTRICTED verdict to X Layer Testnet, and the gated deposit to be
      ...
      {error ? (
        <div className={overviewStyles.errorNote}>
          ...
          <button
            className={overviewStyles.retryButton}
            type="button"
      ...
      {verdict && stage !== "loading" && stage !== "preparing" ? (
        <div
          className={`${styles.resultBlock} ${
            depositBlocked
              ? styles.resultBlockRestricted
              : depositAllowed
                ? styles.resultBlockEligible
                : ""
          }`}
        >
          ...
          <div className={styles.resultGrid}>
            <div className={styles.stateCell}>
              <span className={styles.stateLabel}>NAV</span>
              <span className={styles.stateValue}>
                {quote ? formatFreshness(quote.ageSeconds) : "UNKNOWN"}
              </span>
            </div>
            <div className={styles.stateCell}>
          ...
          {onchainState === "done" && gateway ? (
            <p className={styles.onchainNote}>
              Verdict published to <code>AliveEligibilityRegistry</code> (X
              Layer Testnet, chain 1952):{" "}
              <a
                href={explorerUrl(gateway.publish.txHash)}
                target="_blank"
                rel="noreferrer"
              >
                {gateway.publish.txHash.slice(0, 14)}…
              ...
              {gateway.deposit.ok ? (
                <>
                  ...
                  <a
                    href={explorerUrl(gateway.deposit.txHash)}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {gateway.deposit.txHash.slice(0, 14)}…
                    <ArrowSquareOutIcon size={11} />
                  </a>{" "}
                ...
                <>
                  <code>AliveVault.depositEligibleAsset</code> rejected before
                  broadcast: <code>{gateway.deposit.contractError}</code>.
                </>
              )}
            </p>
          ) : onchainState === "unavailable" ? (
            <p className={styles.onchainNote}>
              Onchain proof not run this session: {onchainUnavailableReason}.
              ALIVE&apos;s detection above (NAV staleness, reason code,
              eligibility status) is real and unaffected; only the X Layer
              publish/deposit step is unavailable here.
            </p>
          ) : null}
        </div>
      ) : null}
