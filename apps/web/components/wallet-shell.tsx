...
export function useWalletAuthorizationSigner():
  ...
  return useMemo(() => {
    ...
    return {
      ...
      sign: async (authorization, domain) =>
...
export function WalletRequirement() {
  ...
  if (!wallet.connected) {
    return (
      <InlineNotice tone="warning" title="Wallet required">
        Connect an injected EVM wallet to bind this operation to its owner.
      </InlineNotice>
    );
  }
  if (!wallet.correctNetwork) {
    return (
      <InlineNotice tone="warning" title="Wrong network">
        Switch to {activeChain.name} before an onchain transaction.
      </InlineNotice>
    );
  }
  return (
    <InlineNotice tone="success" title="Wallet ready">
      {wallet.address ? truncateHash(wallet.address, 12, 8) : "Connected"}
    </InlineNotice>
  );
}
