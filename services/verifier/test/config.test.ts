...
describe("verifier configuration", () => {
  it("keeps default persistence in the monorepo storage boundary", () => {
    ...
    expect(config.databasePath).toBe(
      path.join(workspaceRoot, "storage/database/alive.sqlite"),
    );
    expect(config.evidencePath).toBe(
      path.join(workspaceRoot, "storage/evidence"),
    );
  ...
  it("resolves configured relative persistence paths from the workspace root", () => {
    ...
    const workspaceRoot = path.resolve(process.cwd(), "../..");
    expect(config.databasePath).toBe(
      path.join(workspaceRoot, "storage/database/custom.sqlite"),
    );
    expect(config.evidencePath).toBe(
      path.join(workspaceRoot, "storage/evidence/custom"),
    );
  });
  ...
  it("rejects ambiguous or non-HTTP authorization audiences", () => {
    expect(() =>
      loadVerifierConfig({
        ALIVE_AUTH_AUDIENCE: "http://user:secret@localhost:4100",
      }),
    ).toThrow(/without credentials/i);
    expect(() =>
      loadVerifierConfig({
        ALIVE_AUTH_AUDIENCE: "http://localhost:4100?tenant=one",
      }),
    ).toThrow(/without credentials/i);
  });
});
