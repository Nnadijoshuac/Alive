...
const memoryEvidence: EvidenceStore = {
  async put() { throw new Error("Evidence write should not run before authorization"); },
  async read() { throw new Error("not used"); },
  async reset() {},
};

async function sign(account: typeof ownerAccount, authorization: WalletAuthorization) {
  return account.signTypedData(getAliveAuthorizationTypedData(authorization, authorizationDomain));
}
...
describe("wallet-authorized verifier resources", () => {
  it("creates an asset once, stores only a hashed capability, and protects registration mutations", async () => {
    ...
    const app = await buildApp(config(), { repository, evidenceStore: memoryEvidence, now: () => new Date(instant) });
    const metadata: AssetMetadata = { name: "Authenticated laptop", category: "COMPUTER", model: "A1" };
    ...
    const stored = repository.database
      .prepare("SELECT registration_capability_hash FROM assets WHERE asset_id = ?")
      .get(created.asset.assetId) as { registration_capability_hash: string };
    expect(stored.registration_capability_hash).toBe(hashCapabilityToken(created.capability.token));
    expect(stored.registration_capability_hash).not.toBe(created.capability.token);
    expect(JSON.stringify(repository.database.prepare("SELECT * FROM assets").all())).not.toContain(created.capability.token);
    expect(() => repository.assertAssetCapability(
    ...
    const publicRead = await app.inject({ method: "GET", url: `/api/assets/${created.asset.assetId}` });
    ...
    expect(missingCapability.json()).toMatchObject({ error: { code: "CAPABILITY_REQUIRED" } });
    ...
    expect(wrongCapability.json()).toMatchObject({ error: { code: "CAPABILITY_INVALID" } });
    ...
    expect(replay.json()).toMatchObject({ error: { code: "AUTHORIZATION_ALREADY_USED" } });
    await app.close();
  });

  it("does not consume an authorization for a wrong signature or mutated payload", async () => {
    const repository = new AliveRepository(":memory:");
    const app = await buildApp(config(), { repository, evidenceStore: memoryEvidence });
    ...
    const attackerSignature = await sign(attacker as typeof ownerAccount, challenge.authorization);
    const wrongSigner = await app.inject({
      ...
      payload: {
        ...
        owner,
        metadata,
        authorization: { nonce: challenge.authorization.nonce, signature: attackerSignature },
      },
    });
    expect(wrongSigner.statusCode).toBe(401);
    expect(wrongSigner.json()).toMatchObject({ error: { code: "AUTHORIZATION_SIGNATURE_INVALID" } });
    expect(repository.authorizationConsumedAt(challenge.authorization.nonce)).toBeNull();
    ...
    expect(wrongResource.json()).toMatchObject({ error: { code: "AUTHORIZATION_MISMATCH" } });
    expect(repository.authorizationConsumedAt(challenge.authorization.nonce)).toBeNull();
    ...
    expect(mutated.json()).toMatchObject({ error: { code: "AUTHORIZATION_MISMATCH" } });
    expect(repository.authorizationConsumedAt(challenge.authorization.nonce)).toBeNull();
    await app.close();
  });
  ...
  it("cannot use asset-creation authorization to create a verification session", async () => {
    ...
    const challengeResponse = await app.inject({
      method: "POST",
      url: "/api/auth/challenge",
      payload: { action: "CREATE_ASSET", request: { owner, metadata: testAssetMetadata } },
    });
    const challenge = challengeResponse.json();
    const signature = await sign(ownerAccount, challenge.authorization);
    ...
    expect(response.json()).toMatchObject({ error: { code: "AUTHORIZATION_MISMATCH" } });
    expect(repository.authorizationConsumedAt(challenge.authorization.nonce)).toBeNull();
    await app.close();
  });
  ...
  it("expires authorizations and capabilities according to server time", async () => {
    ...
    const app = await buildApp(
      ...
      { repository, evidenceStore: memoryEvidence, now: () => new Date(instant) },
    );
    const challengeResponse = await app.inject({
      method: "POST",
      url: "/api/auth/challenge",
    ...
    expect(expired.json()).toMatchObject({ error: { code: "AUTHORIZATION_EXPIRED" } });
    await app.close();
  });
  ...
  it("permits exactly one concurrent use of a session authorization and binds its context", async () => {
    ...
    const [left, right] = await Promise.all([
      app.inject({ method: "POST", url: "/api/verifications/session", payload: body }),
      app.inject({ method: "POST", url: "/api/verifications/session", payload: body }),
    ]);
    ...
    expect(repository.authorizationConsumedAt(challenge.authorization.nonce)).not.toBeNull();
    const stored = repository.database
      .prepare("SELECT capability_hash FROM verification_sessions WHERE session_id = ?")
      .get(winner.session.sessionId) as { capability_hash: string };
    expect(stored.capability_hash).toBe(hashCapabilityToken(winner.capability.token));
    expect(stored.capability_hash).not.toBe(winner.capability.token);
    expect(JSON.stringify(repository.database.prepare("SELECT * FROM verification_sessions").all()))
      .not.toContain(winner.capability.token);
    expect(() => repository.assertSessionCapability(
    ...
    for (const endpoint of [
      ...
      { method: "POST", url: `/api/verifications/${winner.session.sessionId}/capture` },
      { method: "POST", url: `/api/verifications/${winner.session.sessionId}/analyze` },
      { method: "POST", url: `/api/verifications/${winner.session.sessionId}/attestation` },
    ] as const) {
      const response = await app.inject(
        endpoint.method === "POST" ? { ...endpoint, payload: {} } : endpoint,
      );
      expect(response.statusCode).toBe(401);
      expect(response.json()).toMatchObject({ error: { code: "CAPABILITY_REQUIRED" } });
    }
    await app.close();
  });
  ...
  it("refuses to authorize another wallet over an authenticated owner's baseline", async () => {
    ...
    expect(response.statusCode).toBe(403);
    expect(response.json()).toMatchObject({ error: { code: "ASSET_OWNER_REQUIRED" } });
    expect(repository.database.prepare("SELECT COUNT(*) AS count FROM wallet_authorizations").get())
      .toEqual({ count: 0 });
    await app.close();
  });
  ...
  it("does not treat pre-authorization legacy asset claims as authenticated owners", async () => {
    ...
    expect(response.json()).toMatchObject({ error: { code: "ASSET_REAUTHORIZATION_REQUIRED" } });
    await app.close();
  });
  ...
  it("revokes the registration capability when fingerprint finalization succeeds", async () => {
    ...
    for (const [index, view] of ["FRONT", "LEFT", "RIGHT", "BACK", "DETAIL"].entries()) {
    ...
    const app = await buildApp(config(), { repository, evidenceStore: memoryEvidence, now: () => new Date(now) });
    ...
    expect(() => repository.assertAssetCapability(assetId, hashCapabilityToken(token), Math.floor(now.getTime() / 1_000)))
      .toThrow(/invalid/i);
    await app.close();
  });
});
