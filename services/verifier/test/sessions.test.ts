...
describe("verification session capabilities", () => {
  ...
  it("expires a session that crosses its deadline during analysis", () => {
    const repository = seededRepository();
    const complete = { ...challenge(0, "38"), completedAt: "2026-01-01T00:00:00.500Z" };
    const created = repository.createSession(
      session({ challenges: [complete], expiresAt: "2026-01-01T00:00:02.000Z" }),
    );
    repository.beginAnalysis(created.sessionId, new Date("2026-01-01T00:00:01.000Z"));
    expect(() =>
      repository.completeAnalysis(created.sessionId, {
        assetId,
    ...
    expect(repository.getSession(created.sessionId, new Date("2026-01-01T00:00:03.000Z"))?.status).toBe("EXPIRED");
    repository.close();
  });
