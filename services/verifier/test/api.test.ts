  ...
  it("reports health and creates an authorized expiring session", async () => {
    ...
    const captureEvidence: EvidenceStore = {
      async put(_namespace, _ownerId, bytes) {
        ...
        return { path: `memory://verification/${index}`, evidenceHash: hashEvidenceBytes(frame), byteLength: frame.byteLength };
      },
      async read() {
        throw new Error("not used");
      },
    ...
    const app = await buildApp(
      ...
      { repository, evidenceStore: captureEvidence, now: () => new Date(fixedNow) },
    );
    ...
    const images = await Promise.all([patternedFrame(0), patternedFrame(2), patternedFrame(4)]);
    ...
    const outOfOrder = await app.inject({
      ...
      headers: { authorization: `Bearer ${created.capability.token}` },
      payload: { challengeId: firstChallengeId, frames: [frames[1], frames[0], frames[2]] },
    });
    expect(outOfOrder.statusCode).toBe(422);
    expect(outOfOrder.json()).toMatchObject({ error: { code: "CAPTURE_SEQUENCE_INVALID" } });
