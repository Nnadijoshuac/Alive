...
export function normalizedIdentifierSimilarity(
  expected: string,
  ...
  for (const candidateValue of candidates) {
    ...
    if (
      candidate.includes(expectedJoined) ||
      expectedJoined.includes(candidate)
    ) {
      best = Math.max(
        best,
        Math.min(candidate.length, expectedJoined.length) /
          Math.max(candidate.length, expectedJoined.length),
      );
      continue;
    }
    ...
    for (
      let expectedIndex = 1;
      expectedIndex <= expectedJoined.length;
      expectedIndex += 1
    ) {
      ...
      for (
        ...
        const substitution =
          diagonal +
          (expectedJoined[expectedIndex - 1] === candidate[candidateIndex - 1]
            ? 0
            : 1);
        rows[candidateIndex] = Math.min(
          (rows[candidateIndex] ?? 0) + 1,
          (rows[candidateIndex - 1] ?? 0) + 1,
          substitution,
        );
        diagonal = previous;
      }
    ...
    const distance =
      ...
      Math.max(expectedJoined.length, candidate.length);
    best = Math.max(
      best,
      1 - distance / Math.max(expectedJoined.length, candidate.length),
    );
  }
...
async function loadTesseract(): Promise<TesseractModule | undefined> {
  ...
  tesseractModule ??= import(packageName)
    .then((module) => module as TesseractModule)
    .catch(() => undefined);
