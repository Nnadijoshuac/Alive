...
export { FileEvidenceStore, decodeCaptureBase64, type EvidenceStore } from "./storage.js";
export { extractViewFingerprint } from "./vision/features.js";
export { analyzeVerification } from "./vision/matching.js";
export { normalizeOcrText, normalizedIdentifierSimilarity } from "./vision/ocr.js";

async function start(): Promise<void> {
  const config = loadVerifierConfig();
  const app = await buildApp(config);
  await app.listen({ host: config.host, port: config.port });
  process.stdout.write(`ALIVE verifier listening on http://${config.host}:${config.port}\n`);
}
