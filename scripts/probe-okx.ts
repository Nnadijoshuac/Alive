...
async function testFetch(url: string, headers: Record<string, string> = {}) {
  try {
...
async function probe() {
  ...
  if (listRes.data && Array.isArray(listRes.data.tokens)) {
    ...
    const xstocks = listRes.data.tokens.filter((t: any) => t.symbol.endsWith("X") || t.name.toLowerCase().includes("stock") || t.name.toLowerCase().includes("backed"));
    console.log("Matching tokens in tokenlist:", xstocks.map((t: any) => ({ symbol: t.symbol, name: t.name, address: t.address })));
  } else {
  ...
  if (chainsRes.data?.data) {
    const xlayer = chainsRes.data.data.find((c: any) => c.chainId === "196" || c.chainId === 196 || c.chainName?.toLowerCase().includes("layer"));
